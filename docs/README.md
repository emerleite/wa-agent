...
|---|---|---|
| v0.18.0 | `AISDKSummarizer` + `AISDKTranscriber` (provider-agnostic drops-in via AI SDK, no `openai` peer needed for transcription anymore), `agentLoopAsAIClient` bridge (route `reply.ai(text)` through `AgentLoop`) | [`PROVIDER_STRATEGY.md`](PROVIDER_STRATEGY.md), [`AGENT_LOOP.md#bridging-agentloop-into-agentai`](AGENT_LOOP.md#bridging-agentloop-into-agentai) |
